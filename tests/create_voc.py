...
parser.add_argument('folder', help='location of folder with images')
# go_pro_icebergs_config.yaml
parser.add_argument('-o', '--output', help='name of vocabulary file')
...
if args.output:
    ...
    image_folder_name = os.path.split(
        os.path.dirname(image_names[0]))[-1].replace(' ', '_')
    detector_config_string = get_detector_config_string(detector)
    voc_file_name = image_folder_name + '_' + \
        str(len(image_names)) + '-' + detector_config_string + '.bin'
...
print("Created Vocabulary {} in {:.2f} secs".format(
    voc_file_name, time.time() - st))
